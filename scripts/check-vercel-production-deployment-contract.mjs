...
async function requireAbsent(relativePath) {
  checkedFiles.add(relativePath);
  try { await fs.stat(path.join(root, relativePath)); errors.push(`Retired production workflow must remain absent: ${relativePath}.`); }
  catch (error) { if (!(error && typeof error === "object" && "code" in error && error.code === "ENOENT")) errors.push(`Unable to verify retired path ${relativePath}: ${error instanceof Error ? error.message : String(error)}`); }
}
function requireTokens(relativePath, source, tokens) { for (const token of tokens) if (!source.includes(token)) errors.push(`${relativePath} is missing exact-deployment token: ${token}`); }
function forbidTokens(relativePath, source, tokens) { for (const token of tokens) if (source.includes(token)) errors.push(`${relativePath} contains prohibited exact-deployment token: ${token}`); }
...
const files = Object.freeze({
  ...
  liveCapabilities: "scripts/verify-live-capability-discovery-current.mjs",
});
...
requireTokens(files.deployer, sources.deployer, [
  ...
  'type: "github-limited"', 'ref: "main"', 'sha: commit', 'target: "production"',
  'deployment.readyState === "READY"', 'deployment.commit === options.commit', 'aliases.includes(PRODUCTION_DOMAIN)',
  '"VERCEL_DEPLOY_COMMIT_UNPROVEN"', '"VERCEL_DEPLOY_SECRET_LEAK"', 'sensitiveValuesRecorded: false',
]);
...
requireTokens(files.exactMain, sources.exactMain, ['git", ["ls-remote", "--heads", "origin", "refs/heads/main"]', 'VECTOR_MAIN_REMOTE_MISMATCH', 'VECTOR_MAIN_REPOSITORY_DIRTY', 'mutationAttempted: false', 'repositoryPublicationAuthority: false']);
forbidTokens(files.exactMain, sources.exactMain, ["git fetch", "git reset", "git checkout", "git switch", "git push"]);
requireTokens(files.sourceProof, sources.sourceProof, ['runChecked("pnpm", ["install", "--frozen-lockfile"]', 'runChecked("pnpm", ["check"]', 'runChecked("pnpm", ["--filter", "@evavo/vector-web", "build"]', 'assertCleanRepository()', 'sensitiveValuesRecorded: false']);
...
requireTokens(files.liveProof, sources.liveProof, ['CANONICAL_ORIGIN = "https://vector.evavo.com.au"', 'LAUNCH_TOKEN_ENV = "VECTOR_DEPLOYMENT_PROOF_LAUNCH_TOKEN"', 'SOURCE_PROOF_ENV = "VECTOR_DEPLOYMENT_SOURCE_PROOF"', '--require-launch', 'sensitiveValuesRecorded: false']);
requireTokens(files.launchToken, sources.launchToken, ['const PROFILE_NAMES = Object.freeze(["owner", "client"])', '"EVAVO_CLIENT_APP_LAUNCH_SECRET"', '"EVAVO_VECTOR_PRIVATE_SIGNING_SECRET"', 'randomBytes(24).toString("base64url")', 'tokenBodyRecorded: false', 'sensitiveValuesRecorded: false', 'mode = 0o600']);
...
requireTokens(files.liveCapabilities, sources.liveCapabilities, [
  ...
  'document.interfaces?.mcp?.contractVersion === "1.6"',
  'document.interfaces?.mcp?.toolCount === 16',
...
forbidTokens(files.liveCapabilities, sources.liveCapabilities, ['contractVersion === "1.5"', 'toolCount === 15', 'managedRemoteExecution === true', 'productionAutoApprovalAvailable === true']);
...
requireTokens(files.orchestrator, sources.orchestrator, [
  ...
  '"scripts/verify-live-capability-discovery-current.mjs"',
  'runLaunchProof("owner"', 'runLaunchProof("client"', 'destroyToken(tokenPath)',
  'launchTokenBodiesRetained: false', 'automaticClientPromotion: false', 'repositoryPublicationAuthority: false', 'githubActionsAuthority: false', 'sensitiveValuesRecorded: false',
]);
forbidTokens(files.orchestrator, sources.orchestrator, ['actions/', 'contents: write', 'git push', 'vercel --prod', 'vercel deploy']);

if (errors.length > 0) { process.stderr.write(`${JSON.stringify({ check: "vector-studio-vercel-production-deployment", ok: false, contractVersion: "2.1", errors }, null, 2)}\n`); process.exit(1); }
process.stdout.write(`${JSON.stringify({ check: "vector-studio-vercel-production-deployment", ok: true, contractVersion: "2.1", providerFreeExecution: true, retiredProductionWorkflowRequiredAbsent: true, exactCurrentMainRequiredBeforeAndAfterProviderEffects: true, sourceProofRequired: true, livePrivateResponseRequired: true, liveCapabilityDiscoveryRequired: true, currentMcpContractVersion: "1.6", currentMcpToolCount: 16, ownerAndClientReplayProofRequired: true, tokenBodiesRetained: false, automaticClientPromotion: false, repositoryPublicationAuthority: "development.repository.publish", checkedFiles: [...checkedFiles].sort() }, null, 2)}\n`);
